...
Q-router 是一个给 OpenClaw 用的本地 OpenAI 兼容路由网关。OpenClaw 本身已经具备基础路由能力，但 Q-router 提供了一个更灵活、可独立演进的增强层，适合需要自定义重试、故障切换、回切策略以及上游控制、且不希望被 OpenClaw 更新节奏绑定的场景。
...
## 为什么 OpenClaw 已经有路由，还要用 Q-router？
OpenClaw 已经有内置路由，但当你希望路由策略和稳定性增强层独立演进时，Q-router 更有价值。它让你可以更细粒度地控制重试策略、fallback / failback 行为、上游实验和本机路由规则，而不用等待 OpenClaw 主体版本更新。
